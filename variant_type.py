"""Identify the molecular consequence of a variant based on its hgvs_c and hgvs_p"""
# Annotates the variants based on the consequences of their mutations.
# Adds the column "Variant_Type" to the final database.
import csv
import re

# Input_file is the file that is used for the database.
# Alternatively it can be used variant_summary.txt as well.
# The first option is better because it contains hgvs_p column.
# The output_file can have the same name as the input_file.
gene = "brca1"
INPUT_FILE = "{gene}_variants.tsv" 
OUTPUT_FILE = "{gene}_variants_full.tsv"
